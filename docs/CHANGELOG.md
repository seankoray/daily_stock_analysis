...
- [新功能] 新增日K/周K BOLL、双模块规则评分 v2 与固定报告展示，并将现有 Portfolio 持仓上下文注入个股分析。
- [新功能] 新增 A 股 5分钟K持久化、15分钟本地聚合、双向做T观察状态机、REST API、常驻监控和 Web 持仓卡片；仅提供辅助建议，不自动下单。
...
- [chore] 将 GitHub Actions 每日股票分析默认定时从北京时间 18:00 调整为 16:00。
