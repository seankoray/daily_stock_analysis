...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_API_URL': {
    ...
    summary: '覆盖 Tushare Pro 请求地址，用于代理渠道。',
    usage: '代理服务商提供专用地址时填写，例如 https://tu.brze.top；留空时使用官方默认地址。',
    valueNotes: ['地址应包含协议，不需要以斜杠结尾。'],
    impact: ['影响主程序 Tushare Pro HTTP 请求和股票列表刷新脚本的 SDK 请求地址。'],
    notes: ['该地址本身不是密钥，真实 token 仍应只放在 TUSHARE_TOKEN。'],
  },
  'settings.data_source.TUSHARE_REQUEST_INTERVAL': {
    title: 'Tushare 请求间隔',
    summary: '控制 Tushare Pro 请求之间的最小秒数。',
    usage: '代理渠道要求控制频次时设置，例如 0.6；留空或 0 表示不额外增加间隔。',
    valueNotes: ['只接受非负数字。'],
    impact: ['影响主程序内置 Tushare HTTP client 的请求频率，降低被代理限流的概率。'],
    notes: ['脚本批量刷新仍保留自身的随机睡眠策略。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_API_URL': {
    ...
    summary: 'Overrides the Tushare Pro endpoint for proxy gateways.',
    usage: 'Set the proxy endpoint, such as https://tu.brze.top. Leave empty to use the official default endpoint.',
    valueNotes: ['Include the protocol. A trailing slash is optional.'],
    impact: ['Affects built-in Tushare Pro HTTP requests and the stock-list refresh script SDK endpoint.'],
    notes: ['This URL is not a secret; keep the real token in TUSHARE_TOKEN.'],
  },
  'settings.data_source.TUSHARE_REQUEST_INTERVAL': {
    title: 'Tushare Request Interval',
    summary: 'Controls the minimum seconds between Tushare Pro requests.',
    usage: 'Use values like 0.6 when a proxy gateway requires throttling. Empty or 0 disables the extra interval.',
    valueNotes: ['Must be a non-negative number.'],
    impact: ['Affects the built-in Tushare HTTP client request frequency and helps avoid proxy rate limits.'],
    notes: ['The batch stock-list script still keeps its own randomized sleep policy.'],
  },
  'settings.data_source.TICKFLOW_API_KEY': {
    title: 'TickFlow API Key',
